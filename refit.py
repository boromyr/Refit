...
facce crollano (test10 333 -> 54, test2 217 -> 116).
...
occhio guardando la mesh, e si misura: su test10 il salto di dimensione fra
...
1. PROTETTI: la Fase A non fonde mai attraverso uno di essi. La regola e'
   ...
   test10: zero tagli falsi, 573 spigoli del CAD protetti.
2. COME CONFINI DI LAVORO: tagliando la mesh dove c'e' uno spigolo vivo
   ...
   originale di test10: le sezioni ritagliano esattamente il cono della
   ...
   QUANTO VALE, in numeri, sempre su test10 contro il CAD vero: degli 8.523
...
danno, su test10 con la Fase A da sola a -a 0.005 e poi -b -c: 171 facce
...
QUANTO CI SI AVVICINA AL CAD DI PARTENZA (test10, misurato)
Il pezzo originale ha 51 facce: 25 piani, 17 cilindri, 6 B-spline, 3 coni.
Con 'refit.py test10.stl -a -b -c -a 0.005' escono 54 facce: 28 piani, 18
...
2. LA TOLLERANZA DELLE FASI B e C - --tol.
   ...
   E' il contrario di quello che dice l'intuito, ed e' misurato. test8, mesh
   grossolana (26.384 triangoli su 112 mm), sempre 'refit.py test8.stl -a -b
...
----------------------------------------------------
Su test8 il 25% dell'area esce ancora tassellata. Guardando dove: 619 delle
...
def texture_barrier(V, fv, nrm, areas, e_faces, size_cut: float = 1.5, ang_min_deg: float = 0.2):
    ...
    misura: su test10 il salto di dimensione fra faccette vale 0.00 (mediana)
    ...
    spigolo. Con "trama > 1.5 E angolo > 0.2 gradi" su test10 i tagli falsi
...
def fit_free(P: np.ndarray, Nrep: np.ndarray, tol: float, check: Optional[np.ndarray] = None, check_tol: Optional[float] = None, max_tilt_deg: float = 70.0) -> Optional[Prim]:
    ...
    su una macchia di test10, 19x17 poli danno 9e-05 sui vertici e 7.6e-02
...
def lm_refine(prim: "Prim", P: np.ndarray, W: Optional[np.ndarray] = None, iters: int = 80) -> "Prim":
    ...
    # non e' dove ci si aspetta. Su test8 il LM e' arrivato a s = 77.8
...
class Segmenter:
    ...
    def _bridges(nF: int, adj):
        ...
        Misurato su test10 contro il file CAD vero: la regola "diedro > 30
    ...
    def sections(self, ang_cut: float = 30.0, size_cut: float = 1.5):
        ...
        facce del pezzo di partenza. Misurato su test10 contro il file CAD
    ...
    def _snap_cylinder(self, p, rf):
        ...
        (su test10: sei coni su nove tornano cilindri, e i tre che restano
...
class Engine:
    ...
    def _safe(self, *a, **k) -> Tuple[bool, str]:
        ...
        tutto il lavoro gia' fatto - su test8 con --tol 0.0005 era un cono
