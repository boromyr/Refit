        ...
        for k in np.argsort(ang, kind="stable"):
            ...
            if float(np.abs((Q - cen) @ nrm_).max()) > lin_tol or slim[k]:
                ...
                if (min(carea[a_], carea[c_]) > 0.2 * max(carea[a_], carea[c_])
                        and not (narrow[pa[k, 0]] and narrow[pa[k, 1]])):
                    continue
                cen2 = Q.mean(axis=0)
                try:
...
def phase_a(shape, lin_tol: Optional[float] = None, ang_tol_deg: float = 0.005,
    ...
    try:
        ...
        sf.Perform()
        fixed = sf.Shape()
        # ⚠️ MA PUO' ANCHE APRIRE IL GUSCIO. Se nel contorno c'e' uno spigolo
        # microscopico (due vertici della mesh a 0.8 micron, capita sul bordo
        # delle macchie a forma libera) ShapeFix lo toglie da UNA faccia e lo
        # lascia nell'altra: lo spigolo resta con una faccia sola e il solido
        # non e' piu' chiuso. Il wire "UnorientableShape" e' un fastidio, un
        # guscio aperto e' un difetto: se apre, si tiene la forma non corretta.
        if fixed is not None and not fixed.IsNull():
            if count_free_edges(fixed) <= count_free_edges(merged):
                merged = fixed
            else:
...
class FreeForm:
    ...
    def point(self, u, v) -> np.ndarray:
        ...
        return (self.C + u[:, None] * self.X + v[:, None] * self.Y
                + self.height(u, v)[:, None] * self.Z)

    def dist(self, P: np.ndarray) -> np.ndarray:
        """Scarto misurato lungo Z: sovrastima quello vero di 1/cos(inclinazione),
        quindi e' un giudice PRUDENTE, mai ottimista."""
        d = np.atleast_2d(P) - self.C
        return (d @ self.Z) - self.height(d @ self.X, d @ self.Y)

    def normal_at(self, P: np.ndarray) -> np.ndarray:
        u, v = self.uv(P)
        h = 1e-4 * max(float(self.ku[-1] - self.ku[0]), 1e-9)
        gu = (self.height(u + h, v) - self.height(u - h, v)) / (2.0 * h)
        gv = (self.height(u, v + h) - self.height(u, v - h)) / (2.0 * h)
        n = (-gu[:, None] * self.X - gv[:, None] * self.Y
             + np.ones((len(np.atleast_1d(u)), 1)) * self.Z)
        return n / np.maximum(np.linalg.norm(n, axis=1), 1e-12)[:, None]
...
def _free_tame(ff: "FreeForm", x, y, z, ngrid: int = 25) -> bool:
    ...
    d2 = ((GX.ravel()[:, None] - x[None, :]) ** 2
          + (GY.ravel()[:, None] - y[None, :]) ** 2).min(axis=1)
...
def fit_free(P: np.ndarray, Nrep: np.ndarray, tol: float,
             probe: Optional[np.ndarray] = None,
             probe_tol: Optional[float] = None,
             max_tilt_deg: float = 70.0) -> Optional[Prim]:
    """
    B-spline ai minimi quadrati su una nuvola di punti con normali.

    tol   : scarto massimo ammesso sui VERTICI della mesh.
    probe : punti che stanno sulla superficie VERA (i baricentri delle faccette
            proiettati sulle primitive dei frammenti che si stanno sostituendo).
            ⚠️ SENZA QUESTI NON SI CAPISCE NIENTE. Una B-spline con troppi poli
            passa per tutti i vertici e fra un vertice e l'altro fa quello che
            vuole: il residuo sui vertici scende a 1e-5 mentre la superficie
            ondeggia di due millimetri. Le sonde sono l'unico modo di guardare
            NEGLI SPAZI VUOTI, e sono gratis: i frammenti che la macchia
            sostituisce sono gia' un modello esatto della superficie, solo a
            pezzi. Si prende la rete di poli PIU' RADA che passa i due esami.
    Ritorna None se non ne passa nessuna: meglio tassellato che inventato.
    """
    ...
    for n in (5, 7, 9, 11, 14, 18):
        if n * n > nmax2:
            break
        try:
            ku = _bs_knots(x0, x1, n)
            kv = _bs_knots(y0, y1, n)
            Bu = _bs_basis(x, ku, n)
            Bv = _bs_basis(y, kv, n)
            A = (Bu[:, :, None] * Bv[:, None, :]).reshape(len(P), n * n)
            D2 = _d2_matrix(n)
            Rg = np.vstack([np.kron(D2, np.eye(n)), np.kron(np.eye(n), D2)])
            sc = float(np.linalg.norm(A)) / max(float(np.linalg.norm(Rg)), 1e-12)
            M = A.T @ A + (1e-8 * sc ** 2) * (Rg.T @ Rg) + 1e-10 * np.eye(n * n)
            c = np.linalg.solve(M, A.T @ z).reshape(n, n)
        except Exception:
            continue
        ...
        if probe is not None and len(probe):
            if float(np.abs(ff.dist(np.asarray(probe, float))).max()) > probe_tol:
                continue
...
class Region:
    ...
    def label(self) -> str:
        ...
        if p.kind == FREE:
            ff = p.free
            return (f"{'LIBERA':<6} {f'{ff.poles.shape[0]}x{ff.poles.shape[1]} poli':<22} "
                    f"{'CONCAVO' if self.concave else 'CONVESSO':<8} {'--':>5} "
                    f"{len(self.faces):>5} facce")
        if p.kind == SPHERE:
            dim = f"ø{2*p.r0:.4f}"
        elif p.kind == TORUS:
...
class Segmenter:
    ...
    def __init__(self, topo, tol_fit: float, tol_grow: float, diag: float,
                 ...
                 allow_torus: bool = True, allow_free: bool = True,
                 blend_angle: float = 20.0, blend_min: int = 3,
                 only_cyl: bool = False, threads: int = 1):
        self.topo = topo
        ...
        self._kw = dict(tol_fit=tol_fit, tol_grow=tol_grow, diag=diag, min_faces=min_faces,
                        ...
                        allow_torus=allow_torus, allow_free=allow_free,
                        blend_angle=blend_angle, blend_min=blend_min,
                        only_cyl=only_cyl, threads=1)
    ...
    def face_tol(self, p, i) -> float:
        ...
        if V.shape[0] < 3:
            return self.tol_fit
        try:
            mids = np.vstack(((V + np.roll(V, -1, axis=0)) * 0.5,
                              self.topo.cents[i][None, :]))
        ...
        for cs in (self.cos_seed, self.cos_seed_wide):
            ...
            for rings, cap in ((1, 40), (2, 80), (4, 250)):
                seed = _ring(s, topo.adj, self.taken, rings, cap=cap,
                             norms=topo.norms, cos_smooth=cs)
    ...
    def _blend_patches(self, regions):
        ...
        def weak(R):
            return (R.prim.kind in (AXIAL, TORUS) and not R.closed_u
                    and R.coverage * 360.0 < self.blend_angle)

        owner = {}
        for k, R in enumerate(regions):
            for i in R.faces:
                owner[i] = k
        wset = {k for k, R in enumerate(regions) if weak(R)}
        if len(wset) < self.blend_min:
            return regions
        seen, drop, extra = set(), set(), []
        for k0 in sorted(wset):
            if k0 in seen:
                continue
            seen.add(k0)
            stack, grp = [k0], []
            while stack:
                k = stack.pop()
                grp.append(k)
                for i in regions[k].faces:
                    for j in topo.adj[i]:
                        q = owner.get(j)
                        if q is not None and q in wset and q not in seen:
                            seen.add(q)
                            stack.append(q)
            if len(grp) < self.blend_min:
                continue
            faces = [i for k in grp for i in regions[k].faces]
            if len(faces) < 4 * self.min_faces:
                continue
            probe = []
            for k in grp:
                pk = regions[k].prim
                Ck = np.array([topo.cents[i] for i in regions[k].faces])
                probe.append(Ck - pk.dist(Ck)[:, None] * pk.normal_at(Ck))
            faces = self._fill_islands(faces, owner)
            P = np.vstack([topo.verts[i] for i in faces])
            N = np.vstack([np.tile(topo.norms[i], (len(topo.verts[i]), 1)) for i in faces])
            pr = fit_free(P, N, self.tol_fit, probe=np.vstack(probe))
            if pr is None:
                continue
            drop |= set(grp)
        ...
        if not extra:
            return regions
        Log.debug(f"Macchie a forma libera: {len(extra)} (al posto di {len(drop)} frammenti)")
    ...
    def _belongs(self, p, i, base) -> bool:
        ...
        sottile ha freccia quasi nulla e resta al micron secco: su test3 le
    ...
    def _wedged(self, found):
        ...
        test3 e' fuori di 1.4 micron da entrambe, e la tolleranza ne concede
        ...
        vede su test3 e test4 vicino a foro, smusso e raccordo.
    ...
    def _absorb_free(self, found):
        ...
        viene rifiutato al commit e le sue faccette restano orfane. Su test4
        ...
        for k in range(len(found)):
            ...
            if p is None or not rf:
                continue
            reg2, _ = grow_region(rf, p, topo.adj, taken, topo.verts, topo.norms,
                                  topo.areas, self.tol_grow, self.cos_ang, refit=False)
            base = set(rf)
            add = [i for i in reg2 if i not in base and not taken[i]
                   and self._belongs(p, i, base)]
            if not add:
                continue
            ...
            keep = set(largest_component(
                [i for i in union if self._belongs(p2, i, base)], topo.adj))
...
def segment_curved(topo: Topo, tol_fit: float, tol_grow: float, diag: float,
                   ...
                   seed_smooth: float = 20.0, seed_smooth_wide: float = 32.0,
                   allow_sphere: bool = True,
                   allow_cone: bool = True, allow_torus: bool = True,
                   allow_free: bool = True, only_cyl: bool = False,
                   threads: int = 1) -> List[Region]:
    return Segmenter(topo, tol_fit, tol_grow, diag, min_faces=min_faces,
                     grow_angle=grow_angle, seed_smooth=seed_smooth,
                     seed_smooth_wide=seed_smooth_wide,
                     allow_sphere=allow_sphere, allow_cone=allow_cone,
                     allow_torus=allow_torus, allow_free=allow_free,
                     only_cyl=only_cyl, threads=threads).run()
...
class Engine:
    ...
    def convert(self, R: Region, strict_hole: bool = False) -> Tuple[bool, str]:
        ...
        if not ok and not strict_hole and ("SelfIntersecting" in why
                                           or "Intersecting" in why):
            ok4, why4 = self._convert(R, strict_hole, fitted_curves=False)
            if ok4:
                ok, why = ok4, why4
    ...
    def _convert(self, R: Region, strict_hole: bool, analytic_planes: bool = True,
                 analytic_curved: bool = True, fitted_curves: bool = True) -> Tuple[bool, str]:
        topo = self.topo
        prim = R.prim
        ...
        if (not strict_hole and len(rf) < 12 and not R.repeated
                and len_debris > 0.6 * max(len_tot, 1e-9)):
            return False, (f"regione piccola e isolata in mezzo a faccette tassellate "
            ...
            for ch in L.chains:
                ...
                if nbp is not None:
                    ...
                    cv = choose_curve(cands, P, self.tol_curve, scale=self.diag,
                                      arc_tol=max(self.tol_curve, 2.0 * R.sag))
...
def run_phase(shape, which: str, tol: Optional[float], max_edge_tol: Optional[float],
    ...
    for round_ in range(2):
        ...
        for k in pending:
            ...
            if not ok and round_ == 0 and ("poligonal" in why or "tolleranza" in why
                                           or "seam" in why or "tassellate" in why):
                again.append(k)
        if not again:
            break
        pending = again
...
def parse_args(argv=None):
    ...
    g.add_argument("--no-free", action="store_true",
                   help="[C] non ricostruire le macchie di raccordo a forma libera "
                        "(B-spline): restano tassellate o spezzate in cilindretti")
